...
printall(){
	...
	for(;;){
		wait 1;
		canprint = [[level.ignore_equipment]](5); 
		if(isdefined(canprint) && canprint){
			iprintlnbold("trickshooters");
		}
	}
}
...
/**
	Utility function for initialising a dvar with the given 
	default value if it is undefined.
 */
setdvarifundefined(dvar, value){
	if(getdvar(dvar) == undefined || getdvar(dvar) == ""){
		setdvar(dvar, value);
	}
}